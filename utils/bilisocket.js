...
class ConnectionPool {
    ...
    reConnect(room_id){
        ...
        this.current_connections[room_id] = client;
        client.onerror = () => {
            this.onConnectionError(client);
        };
        client.onopen = () => {
            this.onConnectionOpen(client, room_id);
        };
        client.onclose = () => {
            this.onConnectionClose(client, room_id);
        };
        this.logging.info("Connection: " + room_id + " RECONNECTED!");
    }
    onConnectionError(client, room_id){
        if(!(room_id in this.current_connections)){
            this.logging.error('Connection Removed (EXPECTED, but caused by error), room id: ' + room_id);
            return ;
        }
        let existedClient = this.current_connections[room_id];
        if(existedClient === undefined){
            this.logging.error('Connection Error happened at creating, room id: ' + room_id);
        }else{
            if (existedClient === client){
                this.logging.error('UNEXPECTED Connection Error happened, room id: ' + room_id);
                let reConnect = this.reConnect;
                setTimeout(function(){
                    reConnect(room_id)
                }, Math.random()*10000);
            }else{
                this.logging.error('Connection Removed (EXPECTED, but caused by duplicated!), room id: ' + room_id);
            }
        }
    }
    onConnectionOpen(client, room_id){
        sendJoinRoom(client, room_id);
        function sendHeartBeat() {
            if (client.readyState === client.OPEN) {
                client.send(HEART_BEAT_PACKAGE);
                setTimeout(sendHeartBeat, 10000);
            }
        }
        sendHeartBeat(true);
    }
    onConnectionClose(client, room_id){
        let existedClient = this.current_connections[room_id];
        if(existedClient && existedClient === client) {
            this.logging.error('Connection UNEXPECTED closed: ' + room_id);
            let reConnect = this.reConnect;
            setTimeout(function () {
                reConnect(room_id)
            }, Math.random() * 10000);
    ...
    addNewConnection = (room_id) => {
        let existedClient = this.current_connections[room_id];
        if(existedClient !== undefined){
            this.logging.error("BAD CODE! do not create duplicated client.");
            return
        }
